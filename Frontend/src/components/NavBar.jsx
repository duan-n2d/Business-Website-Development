...
function NavBar() {
  ...
  useEffect(() => {
    const getBrands = async () => {
      try {
        ...
        const activeBrands = res.data.filter(
  ...
  return (
    <nav className="sticky top-0 w-full z-1000 bg-[#B7EDAB] flex justify-center items-center">
      <div className="w-full md:w-10/12 mx-auto">
        <div className="items-center py-2 my-auto">
          <div
            className={`${
              toggle ? "hidden " : "flex "
            } md:hidden justify-end items-center`}
          >
            <PiListBold className="w-9 h-9" onClick={showNav} />
          </div>
          <div
            className={`${
              toggle ? "flex " : "hidden "
          ...
          >
            <PiXBold className="w-9 h-9" onClick={showNav} />
          </div>
            ...
            {categories.map((category, index) => (
              ...
              >
                <a
                  ...
                  onMouseEnter={() =>
                    handleHover(category.name, category.subItems)
                  }
                ...
                {category.subItems && (
                  ...
                  >
                    {category.subItems.map((item, index) => (
                      <div
                        key={index}
                        className="detail-item pl-5 py-2 hover:bg-green-300"
                      >
          ...
          <ul
            className={`${
              toggle ? "" : "hidden"
            } md:hidden flex-col justify-center items-center w-full first:mt-2 items-center text-[#1B3735]`}
          >
            {categories.map((category, index) => (
              <li
                key={index}
                  ...
                  <div className="">
                    ...
                    {category.subItems.map((item, index) => (
                      <ul
                        ...
                        } flex-col justify-center items-center w-full first:mt-2 items-center text-[#1B3735]`}
                      >
