...
const Card = (product) => {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        const res = await axios.get(API + "product/" + id + "/img");
        setImage((prev) => ({ ...prev, image: res.data}));
      } catch (error) {
        console.error("Error fetching data:", error);
      }
    }
        ...
        <div className="relative">
          ...
          <div className="absolute top-0 left-0 bg-white h-[340px] w-[250px] border border-gray-800">
            ...
            <div className='h-[90px] flex flex-col justify-center items-center text-gray-800 my-auto'>
              <p className='px-1 my-auto text-base text-center font-medium'>{data.product.product_name}</p>
              <p className='text-bold text-red-700 text-center font-medium'>{data.product.current_price}</p>
            </div>
